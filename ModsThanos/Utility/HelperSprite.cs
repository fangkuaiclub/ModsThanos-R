...
namespace ModsThanos.Utility {
    class HelperSprite {
        ...
        public static void ShowAnimation(int TileX, int TileY, bool simpleAnimation, string image, int PixelPerUnit, int framerate, Vector3 position, int LayerValue = 1) {
            GameObject animation = new GameObject { layer = LayerValue };
            animation.transform.position = position;

            var anim = animation.AddComponent<AnimatedTexture>();
            anim.TileX = TileX;
            anim.TileY = TileY;
            anim.simpleAnimation = simpleAnimation;
            anim.image = image;
            anim.PixelPerUnit = PixelPerUnit;
